...
// Generic instruction emission (by Inst::Id).
// ==========================================

namespace {

static ASMJIT_INLINE bool opGp(const Operand_& op) noexcept {
  return op.is_reg() && op.as<Reg>().reg_type() == RegType::kGp64;
}

static ASMJIT_INLINE bool opFp(const Operand_& op) noexcept {
  return op.is_reg() && op.as<Reg>().reg_type() == RegType::kVec64;
}

static ASMJIT_INLINE bool opVec(const Operand_& op) noexcept {
  return op.is_reg() && op.as<Reg>().reg_type() == RegType::kVec128;
}

static ASMJIT_INLINE bool opMem(const Operand_& op) noexcept { return op.is_mem(); }
static ASMJIT_INLINE bool opLabel(const Operand_& op) noexcept { return op.is_label(); }
static ASMJIT_INLINE bool opImm(const Operand_& op) noexcept { return op.is_imm(); }

static ASMJIT_INLINE Error emitInvalidOperand() noexcept { return make_error(Error::kInvalidArgument); }

template<typename T>
static ASMJIT_INLINE T immAs(const Operand_& op) noexcept { return T(op.as<Imm>().value()); }

//! Validates `op_count` operands of `inst_id` against its generated signature.
static ASMJIT_INLINE Error validateInst(InstId inst_id, const Operand_* ops, size_t op_count) noexcept {
  if (ASMJIT_UNLIKELY(inst_id == 0 || inst_id >= Inst::_kIdCount))
    return make_error(Error::kInvalidInstruction);

  const Inst::Signature& sig = Inst::inst_signatures[inst_id];
  if (ASMJIT_UNLIKELY(op_count < sig.required || op_count > sig.count))
    return make_error(Error::kInvalidInstruction);

  for (size_t i = 0; i < op_count; i++) {
    bool ok;
    switch (sig.kinds[i]) {
      case Inst::Kind::kGp:    ok = opGp(ops[i]); break;
      case Inst::Kind::kFp:    ok = opFp(ops[i]); break;
      case Inst::Kind::kVec:   ok = opVec(ops[i]); break;
      case Inst::Kind::kMem:   ok = opMem(ops[i]); break;
      case Inst::Kind::kLabel: ok = opLabel(ops[i]); break;
      case Inst::Kind::kImm:   ok = opImm(ops[i]); break;
      default:                 ok = false; break;
    }
    if (ASMJIT_UNLIKELY(!ok))
...
//! Generic instruction dispatch, generated from the assembler method list.
static Error emitInst(Assembler& asm_, InstId inst_id, const Operand_* ops, size_t op_count) noexcept {
  switch (inst_id) {
    // Generated by tools/ppc_instgen.py - do not edit manually.
    #include <asmjit/ppc/ppc_emitinst.inc>
    default:
      return make_error(Error::kInvalidInstruction);
  }
}
...
Error Assembler::_emit(InstId inst_id, const Operand_& o0, const Operand_& o1, const Operand_& o2, const Operand_* op_ext) {
  ...
  EmitterUtils::op_array_from_emit_args(operands, o0, o1, o2, op_ext);
  size_t op_count = EmitterUtils::op_count_from_emit_args(o0, o1, o2, op_ext);

  // Validate operands against the generated signature table.
  if (has_diagnostic_option(DiagnosticOptions::kValidateAssembler)) {
    Error err = validateInst(inst_id, operands, op_count);
    if (err != Error::kOk) {
      return report_error(err);
    }
  }

  Error err = emitInst(*this, inst_id, operands, op_count);
  if (err == Error::kInvalidInstruction || err == Error::kInvalidArgument) {
    return report_error(err);
  }
  return err;
}
